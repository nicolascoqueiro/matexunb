"""Módulo contendo a lista completa de departamentos e unidades acadêmicas do SIGAA UnB."""
...
def categorize(name: str) -> str:
    ...
    if "CAMPUS" in name_upper:
        return "Campi"
    elif "PROGRAMA DE PÓS-GRADUAÇÃO" in name_upper:
        return "Programas de Pós-Graduação"
    elif "FACULDADE" in name_upper or "INSTITUTO" in name_upper:
        return "Faculdades e Institutos"
    elif "CENTRO" in name_upper or "DECANATO" in name_upper or "DIRETORIA" in name_upper or "REITORIA" in name_upper:
        return "Centros e Decanatos"
    elif "DEPTO" in name_upper or "DEPARTAMENTO" in name_upper:
        return "Departamentos"
    return "Outras Unidades"


SIGAA_DEPARTMENTS: List[Dict[str, str]] = [
    {"code": code, "name": name, "category": categorize(name)}
    for code, name in RAW_DEPARTMENTS
]


def get_departments() -> List[Dict[str, str]]:
    """Retorna a lista completa de departamentos e programas do SIGAA UnB.

    Returns:
        List[Dict[str, str]]: Lista de dicionários contendo code, name e category.
    """
    return SIGAA_DEPARTMENTS


def get_department_name(code: str) -> str:
    """Busca o nome amigável do departamento pelo código.

    Args:
        code (str): Código do departamento no SIGAA.

    Returns:
        str: Nome do departamento ou o próprio código se não localizado.
    """
    for dep in SIGAA_DEPARTMENTS:
        if dep["code"] == str(code):
            return dep["name"]
    return f"Departamento ({code})"
